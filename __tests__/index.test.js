...
describe('Date & Locale utils', () => {
    ...
    describe('toDateTime', () => {
        ...
        it('should have a sub-method tz', () => {
            let result = toDateTime.tz('2000-07-09T22:33:44.222+03:00', 'UTC');
            expect(result).toBeDefined();
            expect(result.isDateTime).toBe(true);
    ...
    describe('toJsDate', () => {
        ...
        it('should handle null and undefined values and other falsy values', () => {
            const values = [null, undefined, '', false, 0];
            expect(values.map((v) => toJsDate(v))).toEqual(values.map(() => undefined));
        });
    });
    ...
    describe('formatToDateOnlyWithLocale', () => {
        ...
        it('should throw on invalid type for locale', () => {
            try {
                formatToDateOnlyWithLocale('2000-02-04T22:33:44', {locale: 1000});
    ...
    describe('formatToDateTimeWithLocale', () => {
        ...
        it('should throw on invalid type for locale', () => {
            try {
                formatToDateTimeWithLocale('2000-02-04T22:33:44', {locale: 1000});
                throw new Error('Should thrown an error');
            } catch (error) {
                expect(error).toBeInstanceOf(Error);
                expect(error.message).toBe('Locale must be an string');
